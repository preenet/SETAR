...
__all__ = ['process_text', 'process_text_old']
...
def process_text_old(text):
    # use for the old bert-base thai
    #pre rules
    res = text.lower().strip()
    res = replace_url(res)
    res = convert_emojis(res)
    ...
    vocab_file = str(Path.joinpath(root, config['models'] + '/' + 'th.wiki.bpe.op25000.vocab' )) 
    spm_file = str(Path.joinpath(root, config['models'] + '/' +'th.wiki.bpe.op25000.model' )) 

    tokenizer = ThaiTokenizer(vocab_file=vocab_file, spm_file=spm_file)
     
    split_sentences = sent_tokenize(res)
    return ' '.join(tokenizer.tokenize(' '.join(split_sentences)))
